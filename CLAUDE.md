...
**Tab bar redesign + fertility calendar bug/guide (2026-07-11 — committed `cead2a1`, pushed, OTA production ✅ update group `f940f0e2`)**:
...
- **Tab bar (`app/(tabs)/_layout.tsx`) — Instagram-სტილი**: `tabBarShowLabel: false` (ლეიბლები მოიხსნა), `TabIcon` ახლა **outline↔filled**-ს ცვლის (`focused ? name : name-outline`), აქტიურზე პატარა წერტილი; ბრტყელი bar (hairline border, `elevation: 0`, height 68→58+inset). **მოცურებული/floating ასისტენტის ღილაკი მოიხსნა** — ახლა ჩვეულებრივი ბრტყელი icon-ია დანარჩენებივით. `tabAccent` fertility-ზე მწვანეა (`#0E9F6E`), თორემ `colors.primary`.
- ⚠️ `_layout.tsx`-ის არსებული lint/tsc შეცდომები (ბრჭყალები admin hint-ში, TS7031 implicit-any) **pre-existing-ია**; ჩემმა ცვლილებამ tsc 8→7 შეამცირა.
