...
**🌐 ენები: ინგლისური + რუსული — i18n ინფრასტრუქტურა (2026-09-21 — **გამოშვებულია**: `main` = 1.0.7, flag `true`; build **1.0.7 (21)** review-ზეა (`f43c20a8`); OTA **1.0.7** ბოლო group `c0d54490`, OTA **1.0.6** group `5768e597` ბრანჩიდან `ota/1.0.6` (version 1.0.6-ზე მიმაგრებული, **`main`-ში არ შეერწყას**; 1.0.6-ის user-ებისთვის მომავალი OTA = `main`-ის merge იქ + გაშვება). picker-ის გარეშე rollback: `update:republish --group 1bd77bd0` (1.0.6). TestFlight-ის შესწორებები: birth picker სქროლი, cycle-length overlap, fertility ბანერის crop, mood label, პირველი splash ინგლისურად, OTA prompt (`hooks/useOtaUpdatePrompt.js`).)**: user-ის გადაწყვეტილება — რუსული ცალკე აპის („Женский календарь", `../Женский календарь`, ქართულის ძველი ფორკი) „დაწევის" ნაცვლად ქართული აპი ერთადერთი codebase ხდება სამივე ენით. **ქართველი user-ისთვის არაფერი იცვლება**: default `ka`, ლექსიკონის ქართული მნიშვნელობები ზუსტად ის string-ებია, რაც კოდში ეწერა.
- **ინფრასტრუქტურა**: `services/i18n.js` (საკუთარი, დამოკიდებულების გარეშე — `t("key", params)`, `{{name}}` ჩასმა, CLDR მრავლობითი `{one,few,many,other}` ka/en/ru-ზე, en/ru-ში ნაკლული key → ქართული fallback; `dayjs.locale`-ს თან ატარებს), `context/LanguageContext.js` (`useLanguage()` → `{t, language, setLanguage, languages, hasChosenLanguage, isLanguageLoaded}`; AsyncStorage `cycle_app_language` + best-effort `profiles.language`), `locales/{ka,en,ru}.js` (~976 key), `locales/native/*.json` + `app.json` → `locales` (Info.plist permission ტექსტები — ⚠️ **build სჭირდება, OTA-თი არ გადის**).
- **Flag**: `constants/tempFlags.js` → **`TEMP_LANGUAGE_PICKER_ENABLED = false`**. `false`-ზე picker არსად არ ჩანს (splash + პროფილის „ენა" row). `true`-ზე: splash 4.5წმ ანიმაციის მერე 3 ღილაკს აჩვენებს **მხოლოდ ახალ ინსტალაციაზე** (სესია არ არის + ენა არ აურჩევია); სესიის მქონე user-ი ქართულზე რჩება, ენას პროფილიდან იცვლის (Alert 3 ვარიანტით; ცვლილებაზე `syncCycleRemindersForUser` თავიდან გეგმავს შეხსენებებს).
- **Guard**: `npm run i18n:check` (`scripts/i18n-check.js`) — (1) ka/en/ru key-ების იდენტურობა, (2) `MIGRATED_FILES`-ში ქართული ასო აღარ არის (line-based, DB-მნიშვნელობები `DB_VALUE_LITERALS`-ით ნებადართულია), (3) **`main`-ის ყველა ქართული string `ka.js`-ში ან კოდში კვლავ არსებობს** (template/emoji/ჩადგმული literal-ების ტოლერანტობით). **ყოველი commit-ის წინ გაუშვი.**
- ⚠️ **ბაზაში ქართული მნიშვნელობები key-ებად დარჩა, migration არ არის**: `profiles.goal` („ციკლის კონტროლი"/„დაორსულება"/„ჯანმრთელობის მონიტორინგი"), `profiles.protection`/`health` (onboarding id-ები), `symptoms.mood` („კარგი"…). ჩვენება `GOAL_LABEL_KEYS`/`moodOptions[].key`/`labelKey`-ით ითარგმნება. **ნუ შეცვლი DB მნიშვნელობებს.**
- **რაც შეიცვალა pattern-ში**: module-level `dayjs.locale("ka")` ყველა tab ეკრანიდან წაშლილია (ენას გლობალურად აბრუნებდა), `utils/dayjs.js`-ს ru/en locale-ები დაემატა; `react-native-calendars`-ის LocaleConfig dayjs-იდან იგება (`applyCalendarLocale`); თვეების/დღეების სახელები dayjs-ს ეკუთვნის, არა ლექსიკონს. სუფთა util-ები (`fertility*`, `ovulationDetection`, `pregnancyMemory`, `dailyAdvice`, `fertilityExport`, `notifications`) module-level `t()`-ს იყენებენ (გამოძახების დროს ითვლება); `PARTNER_TIPS`/`LIFESTYLE_TIPS`/`MUCUS_HINTS` → `getPartnerTips()`/`getLifestyleTips()`/`getMucusHint()`. `dailyAdvice` ვერიფიცირებულია: 2 250 კომბინაციაზე ქართული output ბაიტ-ბაიტ იგივეა.
- **AI** (`assistantOrchestrator.js`): system prompt-ები `__LANGUAGE__` → GEORGIAN/ENGLISH/RUSSIAN `getLanguage()`-ით; prompt-ის ქართული მაგალითები ინგლისურით შეიცვალა (language-neutral); ფაზა/სიმპტომი/განწყობა/მიზანი მოდელს ლექსიკონიდან მიდის; 45წმ context cache ენითაც ინვალიდირდება; ორსულობის კვირეული ბარათი `home.babyDevelopment`/`home.thisWeekAdvice` სათაურებს ითხოვს (`WeeklyAdviceCard` 🍼/💗-ით parse-ავს — უცვლელი). **ლოკალური (AI-ს გარეშე) responder ქართული keyword-ებითაა და ქართულად რჩება** (`// i18n-check: ignore-start/end`); en/ru კითხვები AI-ზე მიდის.
- **შეგნებულად ქართულად დარჩა**: admin UI (`app/admin.jsx`, `services/adminQuery.js`, `_layout.tsx`-ის AdminAssistant), console.log-ები, privacy URL.
- ⚠️ **რუსული აპის („Женский календарь") თარგმანი ნიმუშად არ გამომადგა** — მანქანურია („Маршировать" მარტის ნაცვლად, „авторизоваться" ღილაკზე); რუსული ლექსიკონი თავიდან დავწერე. **მშობლიურის გადახედვა სამედიცინო ტექსტებზე ღირს.**
- **დარჩა**: (1) dev build-ზე გატესტვა flag=`true`-ით (მე ტელეფონზე ვერ ვნახავ; layout რუსულზე გრძელი ტექსტების გამო შესამოწმებელია), (2) ✅ **migration `20260921_add_language_to_profiles.sql` remote-ზე გაშვებულია** (2026-09-21, „women"-ზე, ყველა არსებულ row-ს `ka`; live `main` სვეტს არ ეხება), (3) `main`-ში merge + OTA (picker დახურული) → მერე flag `true` + მეორე OTA, (4) App Store Connect: IAP + listing ლოკალიზაცია EN/RU, ახალი build `locales`-ისთვის, (5) რუსული store-აპის ბედი (ცალკე გადაწყვეტილება: flavor-config + RU Supabase migration-ები).
- **სარეზერვო ასლი**: `../cycle-app - Copy` (2026-09-21 07:27, HEAD `a693605` + Luna-ს uncommitted ცვლილება, node_modules-ით).

**🤖 AI მოდელი `gpt-5.4-mini` → `gpt-5.6-luna` (2026-09-15 — committed `2ccfc84`, **edge function deployed ✅**)**: user-ის გადაწყვეტილება ფასის გამო — Luna $0.20/$1.20 vs mini $0.75/$4.50 (1M ტოკენზე), ე.ი. **~3.75× იაფი** ყველა ფუნქციაზე (ჩატი + ბარათები). ⚠️ **OTA არ სჭირდება** — მხოლოდ სერვერია.
...
npm run i18n:check          # ლოკალიზაციის guard (key parity, ქართული კოდში, nothing lost)
```
