...
- **დარჩა**: (1) dev build-ზე გატესტვა flag=`true`-ით (მე ტელეფონზე ვერ ვნახავ; layout რუსულზე გრძელი ტექსტების გამო შესამოწმებელია), (2) `supabase/migrations/20260921_add_language_to_profiles.sql` remote-ზე გაშვება (სანამ არ არის, ჩაწერა ჩუმად ვარდება — აპი მუშაობს), (3) `main`-ში merge + OTA (picker დახურული) → მერე flag `true` + მეორე OTA, (4) App Store Connect: IAP + listing ლოკალიზაცია EN/RU, ახალი build `locales`-ისთვის, (5) რუსული store-აპის ბედი (ცალკე გადაწყვეტილება: flavor-config + RU Supabase migration-ები).
