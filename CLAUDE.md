...
**🔑 `u@gmail.com` დაემატა ADMIN_EMAILS-ს — სატესტო წვდომა (2026-07-11 — committed `7fde48f`, pushed, OTA production ✅ update group `1dff69d3`)**:
- **პრობლემა**: user-მა ბაზაში ხელით დააყენა `has_pregnancy_subscription=true` fertility-ს გასატესტად; მეორე დღეს გამოირთო და Prime ჩანდა.
...
- **გამოსავალი**: `services/adminAccess.js` → `ADMIN_EMAILS`-ს დაემატა `"u@gmail.com"`. `isAdmin` კოდშია, RevenueCat ვერ გადააწერს → `fertilityUnlocked = isAdmin || hasSubscription` მუდმივად true, `isPremium`-იც (ThemeContext admin-ს ავტომატურად აძლევს).
- ⚠️ **admin email 3 ადგილას არის, შეგნებულად მხოლოდ 1-ში დავამატე**: (1) `services/adminAccess.js` — აპის UI/gate ✅ დამატებულია; (2) `supabase/functions/ai-assistant/index.ts` — უსასრულო AI, **არ დამიმატებია** (Prime-ით ისედაც 20/დღეა, edge deploy დასჭირდებოდა); (3) **DB RLS policies** (`(auth.jwt()->>'email') = 'mishvelidze.u@gmail.com'` — migrations `20260527`, `20260618`) — **განზრახ არ დამიმატებია**: სატესტო ანგარიშს ბაზის დონეზე ყველა user-ის PII-ზე წვდომა არ სჭირდება. ⚠️ ამის შედეგი: `u@gmail.com`-ს admin **პანელი გამოუჩნდება, მაგრამ ცარიელი** (RLS სხვისი row-ები არ დააბრუნებს). ეს მოსალოდნელია — თუ რეალური admin პანელი დასჭირდა, migration საჭიროა.
- 💡 რისკის შეფასება: `u@gmail.com` ანგარიში **უკვე არსებობს** (user-ისაა) → Supabase დუბლიკატს არ დაუშვებს, სხვა ვერ დაიკავებს. Gmail-ზეც ვერ დარეგისტრირდებოდა (6 სიმბოლოზე ნაკლები).
