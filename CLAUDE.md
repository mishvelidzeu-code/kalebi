...
**🔑 `TEST_ACCOUNT_EMAILS` — სატესტო ანგარიში ≠ admin (2026-07-11 — committed `6e316b1`, pushed, OTA production ✅ update group `46496d49`)**: `u@gmail.com`-ს **მხოლოდ ფასიანი რეჟიმები ეხსნება უფასოდ**, სხვა ყველაფერში ჩვეულებრივი user-ია.
...
- **სწორი მექანიზმი**: `services/adminAccess.js` → ახალი `TEST_ACCOUNT_EMAILS` + `isTestAccountEmail()`, ცალკე ADMIN_EMAILS-ისგან (ტესტით გადამოწმებული — სიები არ იკვეთება). `ThemeContext`-ს დაემატა `isTestAccount` (⚠️ **Prime-ს განზრახ არ აძლევს** — admin-ისგან განსხვავებით; user-მა თქვა "როგორც ნებისმიერი სხვა, ერთი გამონაკლისით"). ყველა ფასიანი gate განახლდა: `profile.js` (`freeModeAccess` — fertility row, `handleFertilityEnable`, `handlePregnancyEnable`, `handlePregnancyEntryPress`), `index.js` (home strip), `FertilityContext`, `assistantOrchestrator` (AI persona), `notifications` (fertility შეხსენებები). **`isAdmin` დარჩა მხოლოდ admin UI-ზე** (პანელის ლინკი profile.js:1013, tab-ები).
