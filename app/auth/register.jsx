...
export default function Register() {
  ...
  const { data: onboardingData } = useContext(OnboardingContext); // ვიღებთ კითხვარის პასუხებს
  ...
  const handleRegister = async () => {
    ...
    try {
      // 1. მომხმარებლის შექმნა Supabase-ში
      ...
      if (authData?.user) {
        ...
        // 2. პროფილის შევსება კითხვარის მონაცემებით
        const { error: profileError } = await supabase
          ...
          .upsert({
            ...
            onboarding_completed: true // 👈 აი ეს წყვეტს უსასრულო წრეს!
          });

        if (profileError) throw profileError;

        // 3. პირველი ციკლის ჩაწერა კალენდარისთვის
        ...
        // 4. ყველაფერი მზად არის! გადავდივართ მთავარ ეკრანზე
        router.replace("/(tabs)");
      }
    } catch (error) {
        ...
        <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>
              შექმენი ანგარიში, რომ შენი მონაცემები ყოველთვის დაცული და შენახული იყოს.
            </Text>
          </View>
          ...
          <View style={styles.form}>
            ...
            >
              {loading ? (
                ...
                <Text style={styles.buttonText}>რეგისტრაციის დასრულება 🚀</Text>
              )}
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  button: {
    ...
    alignItems: "center", marginTop: 10, elevation: 8,
    shadowColor: "#ff4d88", shadowOpacity: 0.4, shadowRadius: 15,
  },
